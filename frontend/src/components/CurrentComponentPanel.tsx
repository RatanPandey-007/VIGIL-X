...
export const CurrentComponentPanel: React.FC<CurrentComponentPanelProps> = ({
  ...
  const confidence = component?.forecast?.overall_confidence ? Math.round(component.forecast.overall_confidence) : 82;
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col justify-between space-y-4">
      {/* 1. Header & Identity */}
      <div>
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          ...
          <span className={`px-2.5 py-1 rounded-full text-xs font-mono font-bold flex items-center space-x-1.5 shadow-2xs border ${
            ...
            <span>{isHold ? 'HOLD / REVIEW' : isWatch ? 'WATCH' : 'NOMINAL'}</span>
          </span>
        </div>

        {/* Component & Lot Identification Card */}
        <div className="flex items-center space-x-3.5 bg-slate-50/80 p-3 rounded-xl border border-slate-200/80 mt-3">
      ...
      {/* 2. Section 11: CRUCIAL STATUS DISTINCTION (Absolute Limit vs Lot-Relative) */}
      <div className="bg-gradient-to-b from-slate-50 to-slate-100/60 border border-slate-200 rounded-xl p-3.5 shadow-2xs space-y-2.5">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          {/* Lot-Relative Block */}
          <div className="bg-white p-2.5 rounded-lg border border-amber-300 shadow-2xs">
            <div className="text-[10px] font-mono text-slate-400 font-semibold uppercase">
              LOT-RELATIVE
            </div>
            ...
            <div className="text-[9px] text-amber-700 font-mono mt-0.5 font-semibold">
              &gt;3.5 MAD Divergence
            </div>
          </div>
        </div>

        {/* Supporting Clarification Text */}
        <p className="text-[11px] text-slate-600 font-medium leading-relaxed pt-1 border-t border-slate-200/70">
          &ldquo;Current measurements remain within the configured absolute limit, but behavior is diverging from the lot-normal trajectory.&rdquo;
        </p>
      </div>

      {/* 3. Section 10: Clean Vertical Hierarchy (clean typography and dividers) */}
      <div className="divide-y divide-slate-100 text-xs font-mono">
        ...
        <div className="flex items-center justify-between py-2 text-slate-600">
          ...
          <div className="flex items-center space-x-1.5">
            <span className="font-bold text-amber-800">{lotDevScore}</span>
      ...
      <div className="pt-2 border-t border-slate-100 space-y-2">
        <button
          ...
          className="w-full py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-mono font-bold tracking-tight transition shadow-xs flex items-center justify-center space-x-2 group cursor-pointer"
        >
        ...
        {onInjectDefect && (
          <button
            ...
            className="w-full py-2 px-3 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 text-[11px] font-mono font-semibold transition"
          >
            Inject Latent Drift Test
          </button>
        )}
      </div>
    </div>
